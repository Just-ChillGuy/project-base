...
REDIS_HOST = os.getenv("REDIS_HOST", config.get("redis", {}).get("host", "192.168.56.30"))
REDIS_PORT = int(os.getenv("REDIS_PORT", config.get("redis", {}).get("port", 6379)))

BACKEND_HOST = os.getenv("BACKEND_HOST", config.get("backend", {}).get("host", "192.168.56.20"))
...
def get_user():
    ...
    resp = requests.get(
        f"http://{BACKEND_HOST}:{BACKEND_PORT}/user?id={user_id}",
        timeout=3
    )
